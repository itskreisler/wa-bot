const { configEnv: { BOT_USERNAME }, jsonPrettier } = require('../../helpers/Helpers.cjs')
const createApi = require('../../helpers/create.api.cjs')
const response = require('../../../temp/anime.json')
const { MessageMedia } = require('whatsapp-web.js')
const jsGoogleTranslateFree = require('@kreisler/js-google-translate-free')
const { cursiva, negrita, tachado, monoespaciado } = {
...
module.exports = {
  ...
  async cmd (client, msg, match) {
    const { _data: { notifyName }, from, id: { _serialized: quotedMessageId }, author } = msg
    const [body, accion, q] = match
    ...
    switch (accion) {
      case 'search':
        (async () => {
          ...
          const resultado = data.map(({ mal_id: malId, title }) => monoespaciado(`
🆔MAL_ID: ${malId}
⭐TITLE: ${title}`))
          const message = `
