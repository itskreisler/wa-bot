...
module.exports = {
  ...
  async cmd(client, msg, match) {
    ...
    const msg_chat = await chat.fetchMessages({ limit: 50 })
        ...
        const [animeFLVtask] = client.createTask(_serialized, CRON_JOB, async () => {
          ...
          if (contadorNewEpisodes === 0) {
            ...
            if (botMessages) {
              ...
              const horaNow = dateNow.getHours();
              const diferencia = Math.abs(horaNow - hora);
              ...
              if (diferencia > 5) {
                ...
                console.log(`No es la hora - ${fecha.toLocaleString()}`);
              }
            }
