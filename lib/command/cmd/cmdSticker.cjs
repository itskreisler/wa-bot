    ...
    const sendStickerFromCmd = async (msg) => {
      ...
      try {
        ...
        if (argv.hasOwnProperty('remove') && argv.remove?.length > 0) {
          ...
          const backgroundremover = `python${ifUbuntu} -m backgroundremover.cmd.cli -i "${nameFile}" -m "${m}" -o "${nameFileOutput}"`
          const scripts = {
            bg: backgroundremover,
            bg2: backgroundremover,
            bg3: `python${ifUbuntu} remove_background.py -i "${nameFile}" -o "${nameFileOutput}" -m "${m}"`
